...
namespace UHFPS.Input
{
    ...
    public class InputManager : Singleton<InputManager>
    {
        ...
        public static void Performed(string name, Action<InputAction.CallbackContext> performed)
        {
            ...
            if (action == null)
            {
                if (Instance.debugMode) Debug.LogWarning($"[InputManager] Performed subscription: action '{name}' not found.");
                return;
            }
        ...
        public static IObservable<InputAction.CallbackContext> PerformedObservable(string name)
        {
            InputAction action = Action(name);
            if (action == null)
                return Observable.Empty<InputAction.CallbackContext>();
        ...
        /// Safe: handles missing binding/action gracefully.
        /// </summary>
        public static IObservable<(bool apply, string path)> ObserveBindingPath(string actionName, int bindingIndex)
        {
            // helper to safely get effective path
            Func<string> getSafePath = () =>
            {
                var bp = GetBindingPath(actionName, bindingIndex);
                return bp != null ? bp.EffectivePath : string.Empty;
            };
            ...
            (
                Instance.OnRebindPrepare
                    ...
                    .Select(ctx => (false, ctx.overridePath ?? string.Empty)),
                Instance.OnApply.Select(_ => (true, getSafePath())),
                Instance.OnInputsInit.Select(_ => (true, getSafePath()))
            );
        }
        ...
        public static InputAction Action(string name)
        {
            ...
            // don't spam errors — only warn in debug mode
            if (Instance != null && Instance.debugMode)
                Debug.LogWarning($"[InputManager] Could not find input action with name \"{name}\"!");
        ...
        public static ActionMap.Action ActionMapAction(string name)
        {
            ...
            // don't spam errors — only warn in debug mode
            if (Instance != null && Instance.debugMode)
                Debug.LogWarning($"[InputManager] Could not find action map action with name \"{name}\"!");
        ...
        public static T ReadInput<T>(string actionName) where T : struct
        {
            ...
            if (inputAction == null) return default;
        ...
        public static bool ReadButton(string actionName)
        {
            InputAction inputAction = Action(actionName);
            if (inputAction == null)
                throw new NullReferenceException($"[InputManager] ReadButton: action '{actionName}' not found.");
        ...
        {
            foreach (var toggledAction in Instance.toggledActions.ToList())
            {
        ...
        /// <summary>
        /// Get binding path of action (safe).
        /// </summary>
        public static BindingPath GetBindingPath(string actionName, int bindingIndex = 0)
        {
            ...
            if (!action.bindings.TryGetValue(bindingIndex, out ActionMap.Action.Binding binding))
                return null;
        ...
        public static void StartRebindOperation(string actionName, int bindingIndex = 0)
        {
            ...
            if (action == null)
            {
                if (Instance.debugMode) Debug.LogWarning($"[InputManager] StartRebindOperation: action '{actionName}' not found.");
                return;
            }
        ...
        public static void StartRebindOperation(InputActionReference actionReference, int bindingIndex = 0)
        {
            ...
            if (action == null)
            {
                if (Instance.debugMode) Debug.LogWarning("[InputManager] StartRebindOperation: action reference contains no action.");
                return;
            }
        ...
        public static void SetInputRebindOverrides()
        {
            if (Instance.preparedRebinds.Count > 0)
            {
                ...
                {
                    if (rebind.action == null) continue;
                    ...
                    var bp = GetBindingPath(rebind.action.name, rebind.bindingIndex);
                    if (bp != null)
                        bp.EffectivePath = rebind.overridePath;
                }
        ...
        private void PrepareRebind(RebindContext context)
        {
            ...
            // if context action doesn't exist or binding not found, skip
            if (bindingPath == null)
            {
                if (debugMode) Debug.LogWarning($"[InputManager] PrepareRebind: action '{context.action?.name}' or binding index {context.bindingIndex} not found.");
                return;
            }
            ...
            foreach (XmlNode mapNode in xmlDoc.DocumentElement.ChildNodes)
            {
                foreach (XmlNode actionNode in mapNode.ChildNodes)
                {
                    ...
                    foreach (XmlNode bindingNode in actionNode.ChildNodes)
                    {
                        ...
                        if (action == null)
                        {
                            if (debugMode) Debug.LogWarning($"[InputManager] ReadInputOverrides: action '{actionName}' not found in current input asset. Skipping overrides for it.");
                            continue;
                        }

                        if (!action.bindings.TryGetValue(bindingIndex, out var binding))
                        {
                            if (debugMode) Debug.LogWarning($"[InputManager] ReadInputOverrides: binding index {bindingIndex} not found for action '{actionName}'.");
                            continue;
                        }
        ...
        public class RebindContext
        {
            ...
            {
                string actionName = action?.name ?? "<null>";
