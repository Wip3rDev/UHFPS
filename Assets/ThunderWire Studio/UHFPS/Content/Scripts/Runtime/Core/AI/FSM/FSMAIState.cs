...
namespace UHFPS.Runtime
{
    public class FSMAIState : FSMState
    {
        ...
        protected NPCWeaponHandler weaponHandler;
        ...
        public FSMAIState(NPCStateMachine machine)
        {
            ...
            agent = machine.Agent;
            weaponHandler = machine.GetComponent<NPCWeaponHandler>();
        ...
        /// <summary>
        /// Does the AI hear the player (player is running in hearing radius) or was alerted by damage?
        /// </summary>
        public bool HearsPlayerOrAlerted()
        {
            if (machine.PerceptionSystemInstance == null)
                return false;

            return machine.PerceptionSystemInstance.ShouldChasePlayer();
        }

        /// <summary>
        /// Does the AI see the player, hear the player, or was alerted by damage?
        /// </summary>
        public bool SeesOrHearsPlayer(float closeDistance = 0f)
        {
            return SeesPlayerOrClose(closeDistance) || HearsPlayerOrAlerted();
        }
