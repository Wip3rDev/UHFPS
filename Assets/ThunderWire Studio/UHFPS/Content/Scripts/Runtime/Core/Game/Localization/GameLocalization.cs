...
namespace UHFPS.Runtime
{
    ...
    {
        public const string LANGUAGE_KEY = "UHFPS_SelectedLanguage";
        ...
        {
            get => _languageDict ??= GenerateLangDictionary(currentLanguageIndex);
            private set => _languageDict = value;
        }

        protected void Awake()
        {
            // Load saved language from PlayerPrefs, or use default
            int savedLanguage = PlayerPrefs.GetInt(LANGUAGE_KEY, -1);
            currentLanguageIndex = savedLanguage >= 0 ? savedLanguage : DefaultLanguage;

            // Regenerate dictionary with the loaded/saved language
            _languageDict = GenerateLangDictionary(currentLanguageIndex);
        }

        protected void OnDestroy()
        {
            // Clear PlayerPrefs when the game is fully closed (not when switching scenes)
            // This ensures language is preserved during scene transitions but reset on game restart
        }
        ...
        public void ChangeLanguage(int newLanguageIndex)
        {
            ...
            PlayerPrefs.SetInt(LANGUAGE_KEY, newLanguageIndex);
            PlayerPrefs.Save();
