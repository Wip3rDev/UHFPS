...
namespace UHFPS.Runtime
{
    ...
    public class ObjectiveManager : Singleton<ObjectiveManager>, ISaveableCustom
    {
        ...
        // Flag to prevent notifications during game loading
        ...
        public void AddObjective(string key, params string[] subKey)
        {
            ...
            // Show notification only if not loading game
            if (!IsLoadingGame)
            {
        ...
        {
            // Set loading flag to prevent notifications during load
            IsLoadingGame = true;
            ...
            foreach (var obj in objectives.Properties())
            {
                ...
                // Get sub objective keys
                string[] subObjectiveKeys = subObjectives.Properties().Select(x => x.Name).ToArray();

                // Check if objective already exists in activeObjectives (added during gameplay)
                ObjectiveData objectiveData;
                bool isNewObjective = !activeObjectives.TryGetValue(obj.Name, out objectiveData);

                if (isNewObjective)
                {
                    // Create new objective data if it doesn't exist
                    objectiveData = CreateObjectiveData(obj.Name, subObjectiveKeys, false);
                ...
                // Update the objective state (works for both new and existing objectives)
                if (objectiveData != null)
                {
                    // Instead of replacing the BehaviorSubject, just set its value
                    // This keeps the subscription working!
                    objectiveData.IsCompleted.OnNext(isCompleted);
                    ...
                    foreach (var sub in subObjectives.Properties())
                    {
                        ...
                        if (objectiveData.SubObjectives.TryGetValue(sub.Name, out var subData))
                        {
                            // Instead of replacing the BehaviorSubject, just set its value
                            // This keeps the subscription working!
                            subData.CompleteCount.OnNext(completeCount);
                            subData.IsCompleted.OnNext(isSubCompleted);
                        }
                    }
                }
            }
        ...
        public struct ObjectiveCache
        {
            ...
            public ObjectiveCache(Objective objective)
            {
                ...
                // 🔹 Локализация убрана из конструктора!
                // Теперь подписка на GameLocalization происходит в ObjectiveHolder

                // 🔹 Заполняем словарь подзадач по ключу
