    ...
    public class DialogueSystem : Singleton<DialogueSystem>
    {
        ...
        private IEnumerator HandleSubtitles(Dialogue dialogue)
        {
            ...
            if (dialogue.SubtitleType == SubtitleTypeEnum.Single)
            {
                ...
                SendBinderEvent(DialogueBinderType.Subtitle, new object[] { dialogue.DialogueAudio, GetLocalizedText(subtitle.Text) });
                    ...
                    for (int i = subtitleIndex + 1; i < sortedSubtitles.Count; i++)
                    {
                        ...
                        if (time > subtitle.Time)
                        {
                            if(subtitle is DialogueSubtitle sub)
                            {
                                ...
                                {
                                    // send subtitle event to binder
                                    SendBinderEvent(DialogueBinderType.Subtitle, new object[] { dialogue.DialogueAudio, GetLocalizedText(sub.Text) });
                                    subtitleIndex = i;
                                }
                            }
        ...
        {
            // Get the localized text from GString
            string localizedText = GetLocalizedText(subtitle.Text);
            ...
            if (UseNarratorColors)
            {
                ...
                string text = ShowNarrator && !subtitle.Narrator.IsEmpty()
                    ? $"<b><color=#{color}>{subtitle.Narrator}: </color></b> {localizedText}"
                    : localizedText;
            ...
            {
                string text = ShowNarrator && !subtitle.Narrator.IsEmpty()
                    ? $"<b>{subtitle.Narrator}: </b> {localizedText}"
                    : localizedText;
        ...
        private string GetLocalizedText(GString gstring)
        {
            if (gstring == null)
                return string.Empty;

#if UHFPS_LOCALIZATION
            // If GlocText is set and not just a normal text marker
            if (!string.IsNullOrEmpty(gstring.GlocText) && gstring.GlocText[0] != '*')
            {
                // Try to get localized text from the dictionary
                if (GameLocalization.HasReference && 
                    GameLocalization.Instance.LanguageDict.TryGetValue(gstring.GlocText, out string localizedText))
                {
                    return localizedText;
                }
                // If key not found, return the key itself (fallback behavior)
                return gstring.GlocText;
            }
#endif
            // Return NormalText (which may be the actual text or fallback)
            return gstring.NormalText ?? gstring.GlocText;
        }
