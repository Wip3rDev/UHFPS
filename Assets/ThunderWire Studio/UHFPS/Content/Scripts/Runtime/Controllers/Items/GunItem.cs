...
namespace UHFPS.Runtime
{
    public class GunItem : PlayerItemBehaviour
    {
        ...
        private void FireOneBullet()
        {
            ...
            // Notify NPCs about the shot
            if (bulletAndMuzzleFlash.BarrelEnd != null)
                PlayerPresenceManager.OnPlayerShot?.Invoke(bulletAndMuzzleFlash.BarrelEnd.position);
        ...
        IEnumerator HideGun()
        {
            ...
            // IMPORTANT: Reset FOV at the end of animation, after isEquipped = false
            // This is needed because OnUpdate() continues to run during animation
            // and if ADS button is still held, it keeps changing FOV
            if (gunProperties.EnableAdsFov)
            {
                PlayerManager.MainVirtualCamera.Lens.FieldOfView = defaultFov;
            }
